...
namespace Yukar.Battle
{
    ...
    class BattleContentGetter : BattleContentGetterBase
    {
            ...
            GameContentCreator owner, RenderContent result, ref GameContentParser.StatusUpDown refStatusUpDown)
        {
            ...
            switch (content.type)
            {
                ...
                case GameContentParser.ContentType.ENEMYSUBCLASSLEVEL:
                    {
                        ...
                        if (heros.Count > index)
                        {
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Party;
                ...
                case GameContentParser.ContentType.LEVELUPNAME:
                    {
                        var chr = gameContent.CharacterLevelUpData.Count > index ? gameContent.CharacterLevelUpData[index] : null;
                        if (chr?.player != null)
                        {
                            result.ElementType = SpecialTextRenderer.SpecialTexts.LevelUp;
                            return chr.player.Name;
                        }
                ...
                case GameContentParser.ContentType.LEVELUPBEFORESTATUS_A:
                    {
                        ...
                        if (chr?.player != null)
                        {
                            var hero = chr.player.player;
                            var cast = hero.rom;
                            var level = chr.castLevelUpData.CurrentLevel;
                            ...
                            if (content.growthFactor)
                            {
                                ...
                                if (CalclateHeroGrowthFactor(hero, cast, preLevel, content.attrs[0], chr, catalog, gameContent, ref preStatus))
                                {
                                    result.ElementType = SpecialTextRenderer.SpecialTexts.LevelUp;
                                    return preStatus.ToString();
                                }
#else
                                GetPreLevel(chr, out int lv, out int jobLv, out int sideJobLv);
                                if (GetHeroGrowthFactor(hero, cast, lv, jobLv, sideJobLv, index, chr, catalog, out int preStatus))
                                {
                                    result.ElementType = SpecialTextRenderer.SpecialTexts.LevelUp;
                                    return preStatus.ToString();
                                }
                            ...
                            {
#if !USE_JOB_CACHE
                                var preStatus = CalclateHeroStatus(hero, cast, preLevel, chr, catalog, gameContent);
...
#else
                                GetPreLevel(chr, out int lv, out int jobLv, out int sideJobLv);
                                var preStatus = GetHeroStatus(hero, cast, lv, jobLv, sideJobLv, chr, catalog);
                                if (preStatus.Length > content.attrs[0])
                                {
                                    result.ElementType = SpecialTextRenderer.SpecialTexts.LevelUp;
                                    return preStatus[content.attrs[0]].ToString();
                ...
                case GameContentParser.ContentType.LEVELUPAFTERSTATUS_A:
                    {
                        ...
                        if (chr?.player != null)
                        {
                            ...
                            if (content.growthFactor)
                            {
                                ...
                                if (CalclateHeroGrowthFactor(hero, cast, level, content.attrs[0], chr, catalog, gameContent, ref nowStatus))
                                {
                                    var preStatus = 0;

                                    CalclateHeroGrowthFactor(hero, cast, preLevel, content.attrs[0], chr, catalog, gameContent, ref preStatus);
...
#else
                                if (GetHeroGrowthFactorPrePost(hero, cast, chr, catalog, content.attrs[0], out int preStatus, out int nowStatus))
                                {
                                    refStatusUpDown = CompareStatus(preStatus, nowStatus);
                                    result.ElementType = SpecialTextRenderer.SpecialTexts.LevelUp;
                                    return nowStatus.ToString();
                            ...
                            {
#if !USE_JOB_CACHE
                                var nowStatus = CalclateHeroStatus(hero, cast, level, chr, catalog, gameContent);
...
#else
                                if (GetHeroStatusPrePost(hero, cast, chr, catalog, content.attrs[0], out int preVal, out int nowVal))
                                {
                                    refStatusUpDown = CompareStatus(preVal, nowVal);
                                    result.ElementType = SpecialTextRenderer.SpecialTexts.LevelUp;
                                    return nowVal.ToString();
                                }
#endif
                            }
                        }
                        ...
                        if (chr?.player != null)
                        {
                            ...
                            if (content.growthFactor)
                            {
                                ...
                                if (CalclateHeroGrowthFactor(hero, cast, preLevel, content.attrs[0], chr, catalog, gameContent, ref preStatus))
                                {
                                    ...
                                    CalclateHeroGrowthFactor(hero, cast, level, content.attrs[0], chr, catalog, gameContent, ref nowStatus);
                                    ...
                                    if (diff != 0)
                                    {
                                        ...
                                        var arrow = GameMain.instance.catalog.getGameSettings().glossary.Arrow;
                            ...
                            else
                            {
                                ...
                                if (GetHeroStatusPrePost(hero, cast, chr, catalog, content.attrs[0], out int preVal, out int nowVal))
                                {
                                    var diff = nowVal - preVal;
                                    if (diff != 0)
                                    {
                                        refStatusUpDown = CompareStatus(preVal, nowVal);
                                        var arrow = GameMain.instance.catalog.getGameSettings().glossary.Arrow;
                                        result.ElementType = SpecialTextRenderer.SpecialTexts.LevelUp;
                                        return arrow + nowVal.ToString();
                                    }
                                }
#endif
                            }
                ...
                case GameContentParser.ContentType.LEVELUPDIFFSTATUS_A:
                    {
                        ...
                        if (chr?.player != null)
                        {
                            ...
                            else
                            {
                                ...
                                if (GetHeroStatusPrePost(hero, cast, chr, catalog, content.attrs[0], out int preVal, out int nowVal))
                                {
                                    var arrow = GetStatusDiffText(preVal, nowVal, out refStatusUpDown);
                                    result.HasLevelUpElement = (refStatusUpDown != GameContentParser.StatusUpDown.NoChange);
                        ...
                        if (chr?.player != null)
                        {
                            ...
                            if (content.growthFactor)
                            {
                                ...
                                if (CalclateHeroGrowthFactor(hero, cast, preLevel, content.attrs[0], chr, catalog, gameContent, ref preStatus))
                                {
                                    ...
                                    var diff = nowStatus - preStatus;
                                    var easingTime = chr.easingProgress;
                                ...
                                { 
                                    var diff = nowStatus - preStatus;
                                    var diffEasingValue = (int)(SimpleEasing(chr.easingProgress) * diff);
...
#else
#if DEBUG
                                if (content.attrs[0] == 1)
                                {
                                    DumpStats(hero, cast, chr, catalog);
                                }
#endif
                                if (GetHeroStatusPrePost(hero, cast, chr, catalog, content.attrs[0], out int preVal, out int nowVal))
                                {
                                    ...
                                    result.ElementType = SpecialTextRenderer.SpecialTexts.LevelUp;
                                    return (preVal + diffEasingValue).ToString();
                                }
#endif
                            }
                        }
                ...
                case GameContentParser.ContentType.ITEMNAME:
                    {
                        ...
                        {
                            result.ElementType = SpecialTextRenderer.SpecialTexts.Item;
        ...
        private static float SimpleEasing(float easingTime)
        {
        //  if (0.35f > easingTime) easingTime = 0f;
        //  else if (easingTime > 0.65f) easingTime = 1.0f;
        //  else easingTime = (easingTime - 0.35f) * 3.33f;
        //  const float sec = 1.0f;
        //  var res = Engine.Easing.EasingFunction.GetEasingResult(new Engine.Easing.Linear(), easingTime, sec, 0.0f, 1.0f);

            const float totalSec = 1.0f;
            const float startDelay = 0.35f;
            const float stopDelay = 0.35f;
            const float actionSec = totalSec - (startDelay + stopDelay);
            System.Diagnostics.Debug.Assert(actionSec > 0.0f);

            if (easingTime < startDelay)
            {
                easingTime = 0.0f;
            }
            else
            if (easingTime < startDelay + actionSec)
            {
                easingTime = (easingTime - startDelay) / actionSec;
            }
            else
            {
                easingTime = 1.0f;
            }
            var res = Engine.Easing.EasingFunction.GetEasingResult(new Engine.Easing.Linear(), easingTime, totalSec, 0.0f, 1.0f);

            return res;
        }

        private static void GetCurrentLevel(CharacterLevelUpData levelUpData, out int lv, out int jobLv, out int sideJobLv)
        {
            lv = levelUpData?.castLevelUpData.CurrentLevel ?? 0;
            jobLv = levelUpData?.jobLevelUpData.CurrentLevel ?? 0;
            sideJobLv = levelUpData?.sideJobLevelUpData.CurrentLevel ?? 0;
        }

        private static void GetPreLevel(CharacterLevelUpData levelUpData, out int lv, out int jobLv, out int sideJobLv)
        {
            lv = levelUpData?.castLevelUpData.preLevel ?? 0;
            jobLv = levelUpData?.jobLevelUpData.preLevel ?? 0;
            sideJobLv = levelUpData?.sideJobLevelUpData.preLevel ?? 0;

            if (lv < 0 || jobLv < 0 || sideJobLv < 0)
            {
                GetCurrentLevel(levelUpData, out lv, out jobLv, out sideJobLv);
            }
        }

        private static Common.GameData.CastJob GetCastJobByLevel(Catalog catalog, Common.GameData.Hero hero, CharacterLevelUpData levelUpData, int level, bool isSideJob)
        {
            Common.GameData.CastJob castJob = levelUpData?.characterStatsCache.GetCastJob(level, isSideJob);

            if (castJob == null)
            {
                int exp = 0;
                var rom = isSideJob? (hero?.sideJobCast?.rom) : (hero?.jobCast?.rom);
                var tbl = rom?.levelUpExpList;

                if (tbl != null && level > 1 && level < tbl.Count)
                {
                    exp = tbl[level - 2];
                }

                castJob = Common.GameData.Party.createCastJobFromRom(catalog, rom, isSideJob, exp); // 重いので注意 / Be careful as it's heavy
                if (castJob != null)
                {
                //  System.Diagnostics.Debug.Assert(castJob.level == level);
                    levelUpData?.characterStatsCache.Add(level, isSideJob, castJob);
                }
            }
        ...
        private static bool CalclateHeroGrowthFactor(Common.GameData.Hero hero, Cast cast, int level, int jobLevel, int sideJobLevel, int index, 
                                                        CharacterLevelUpData clud, Catalog catalog, ref int refParam)
        {
            var info = catalog.getGameSettings().GetCastStatusParamInfo(index);
            ...
            var paramList = cast.StatusParam.ParamDic[info.guId].Item2;
            level = (level <= paramList.Count) ? level : paramList.Count;

            refParam = paramList[level - 1] + hero.itemEffect.statusValue.GetStatus(info.guId);

        //  refParam += Common.GameData.Party.createCastJobFromRom(catalog, hero.jobCast?.rom, false, clud.jobLevelUpData.totalExp)?.statusValue.GetStatus(info.guId) ?? 0;
        //  refParam += Common.GameData.Party.createCastJobFromRom(catalog, hero.sideJobCast?.rom, true, clud.sideJobLevelUpData.totalExp)?.statusValue.GetStatus(info.guId) ?? 0;

            var jobCast = GetCastJobByLevel(catalog, hero, clud, jobLevel, false);
            var sideJobCast = GetCastJobByLevel(catalog, hero, clud, sideJobLevel, true);

            refParam += jobCast?.statusValue.GetStatus(info.guId) ?? 0;
            refParam += sideJobCast?.statusValue.GetStatus(info.guId) ?? 0;

            return true;
        }

        private static int[] CalclateHeroStatus(Common.GameData.Hero hero, Cast cast, int level, int jobLevel, int sideJobLevel, 
                                                    CharacterLevelUpData clud, Catalog catalog)
        {
//          var chr = gameContent.CharacterLevelUpData.FirstOrDefault(x => x.player.player.rom.guId == cast.guId);
//
//          // TODO: chr == cludなのでは？
// // TODO: Isn't it chr == clud?
//          if (chr.player.player.rom.guId == clud?.player.player.rom.guId)
//          {
//              Console.WriteLine("CalclateHeroStatus:match");
//          }
//          else
//          {
//              Console.WriteLine("CalclateHeroStatus:unmatch!!!!");
//          }
//
//          chr = clud;   // TODO:様子見 -> gameContentが要らなくなる -> gameContent削除
// chr = clud; // TODO: Waiting and seeing -> gameContent no longer needed -> gameContent deleted

//          if (clud == null)
//          {
//          //  return new int[20];
//              return (new CharacterStats()).GetArray();
//          }

            var status = new int[] {
                level,
                hero.calcStatus(cast.hpParam, level, true) + hero.itemEffect.maxHitpoint,
                clud?.player.HitPoint ?? 0,
                hero.calcStatus(cast.mpParam, level) + hero.itemEffect.maxMagicpoint,
                clud?.player.MagicPoint ?? 0,
                hero.calcStatus(cast.powerParam, level) + hero.itemEffect.power,
                hero.calcStatus(cast.vitalityParam, level) + hero.itemEffect.vitality,
                hero.calcStatus(cast.magicParam, level) + hero.itemEffect.magic,
                hero.calcStatus(cast.speedParam, level) + hero.itemEffect.speed,
                clud?.castLevelUpData.totalExp ?? 0,
                clud?.castLevelUpData.CurrentNextLevelNeedExp ?? 0,
                hero.calcStatus(cast.powerParam, level) + hero.attackBase,
                clud?.player.ElementAttack ?? 0,
                hero.calcStatus(cast.vitalityParam, level) + hero.defenseBase,
                Math.Min(100, clud?.player.Dexterity ?? 0),
                Math.Min(100, clud?.player.Evasion ?? 0),
                Math.Min(100, clud?.player.Critical ?? 0),
                0,
                hero.maxLevel,
                clud?.castLevelUpData.CurrentExp ?? 0,
            };

        //  var jobStatus = GetJobStatus(Common.GameData.Party.createCastJobFromRom(catalog, hero.jobCast?.rom, false, clud.jobLevelUpData.totalExp));
        //  var sideJobStatus = GetJobStatus(Common.GameData.Party.createCastJobFromRom(catalog, hero.sideJobCast?.rom, true, clud.sideJobLevelUpData.totalExp));

            var castJob = GetCastJobByLevel(catalog, hero, clud, jobLevel, false);
            var castSideJob = GetCastJobByLevel(catalog, hero, clud, sideJobLevel, true);

            var jobStatus = GetJobStatus(castJob);
            var sideJobStatus = GetJobStatus(castSideJob);
        ...
        private static int[] GetHeroStatus(Common.GameData.Hero hero, Cast cast, 
                                            int castLv, int jobLv, int sideLv, 
                                            CharacterLevelUpData levelUpData, Catalog catalog)
        {
            int[] status = levelUpData?.characterStatsCache.GetStats(castLv, jobLv, sideLv)?.GetArray() ?? null;

            if (status == null)
            {
                status = CalclateHeroStatus(hero, cast, castLv, jobLv, sideLv, levelUpData, catalog);
                if (status != null)
                {
                    levelUpData?.characterStatsCache.Add(castLv, jobLv, sideLv, new CharacterStats(status));
                }
            }
        ...
        private static bool GetHeroStatusPrePost(Common.GameData.Hero hero, Cast cast, CharacterLevelUpData chr, Catalog catalog,
                                                    int index,
                                                    out int preVal, out int postVal)
        {
            GetCurrentLevel(chr, out int lv, out int jobLv, out int sideJobLv);
            var nowStatus = GetHeroStatus(hero, cast, lv, jobLv, sideJobLv, chr, catalog);

        //  if (chr.onLevelUpEffect)
            {
                GetPreLevel(chr, out lv, out jobLv, out sideJobLv);
            }
            var preStatus = GetHeroStatus(hero, cast, lv, jobLv, sideJobLv, chr, catalog);

            if (index >= preStatus?.Length ||
                index >= nowStatus?.Length )
            {
                preVal = 0;
                postVal = 0;
                return false;
            ...
            preVal = preStatus[index];
            postVal = nowStatus[index];
            return true;
        }

        private static void DumpStats(Common.GameData.Hero hero, Cast cast, CharacterLevelUpData chr, Catalog catalog)
        {
#if DEBUG
#if false
            var progress = chr.easingProgress;

            Action<LevelUpData, string> dumpJob = (job, tag) =>
            {
                var _lv = job.CurrentLevel;
                var exp = job.totalExp;
                var addExp = job.addExp;
                var uplv = job.upLevel;
                var prelv = job.preLevel;
                var isUp = job.IsLevelUp;
                Console.WriteLine($"{tag}:[level:{_lv}(up:{uplv})(pre:{prelv})][exp:{exp}/{addExp}][isUp:{isUp}]");
            };

            Console.WriteLine($"TEST:[{cast.Name}][per:{progress:F5}]");
            dumpJob(chr.castLevelUpData, "CAS");
            dumpJob(chr.jobLevelUpData, "JOB");
            dumpJob(chr.sideJobLevelUpData, "SUB");

            GetCurrentLevel(chr, out int lv, out int jobLv, out int sideJobLv);
            var postStats = new CharacterStats(GetHeroStatus(hero, cast, lv, jobLv, sideJobLv, chr, catalog));

            GetPreLevel(chr, out lv, out jobLv, out sideJobLv);
            var preStats = new CharacterStats(GetHeroStatus(hero, cast, lv, jobLv, sideJobLv, chr, catalog));
        ...
        private static bool GetHeroGrowthFactor(Common.GameData.Hero hero, Cast cast, 
                                                int castLv, int jobLv, int sideLv, int index, 
                                                CharacterLevelUpData levelUpData, Catalog catalog,
                                                out int refParam)
        {
            int param = 0;

            if (levelUpData?.characterStatsCache.GetParam(castLv, jobLv, sideLv, index, out param) ?? false)
            {
                refParam = param;
                return true;
            }
            ...
            {
                int res = 0;
                if (CalclateHeroGrowthFactor(hero, cast, castLv, jobLv, sideLv, index, levelUpData, catalog, ref res))
                {
                    levelUpData?.characterStatsCache.AddParam(castLv, jobLv, sideLv, index, res);
                    refParam = res;
                    return true;
                }
            }
        ...
        private static bool GetHeroGrowthFactorPrePost(Common.GameData.Hero hero, Cast cast, CharacterLevelUpData chr, Catalog catalog,
                                                    int index,
                                                    out int preVal, out int postVal)
        {
            preVal = 0;
            ...
            if (!GetHeroGrowthFactor(hero, cast, lv, jobLv, sideJobLv, index, chr, catalog, out int nowStatus))
            {
                return false;
            }
            postVal = nowStatus;
            ...
            if (gameMain?.mapScene?.isBattle ?? false)
            {
                ...
                switch (content.type)
                {
                    ...
                    case GameContentParser.ContentType.ENEMYSPPERCENT_A:
#if false
                        // 置き換え様子見中 [20250509]
                        // Waiting and watching replacement [20250509]
                        ...
                        return true;
#else
                        if ((content.attrs.Count > 0) && (menuIndex > -1))
                        {
                            var no = content.attrs[0];
                            var catalog = gameMain.catalog;
                            bool? isHP = null;
                            ...
                            if (content.growthFactor)
                            {
                                ...
                                if (info != null)
                                {
                                    isHP = (info.guId == gs.maxHPStatusID);
                                }
                            }
                            else
                            {
                            ...
                            if (isHP.HasValue)
                            {
                                ...
                                {
                                    var enemy = enemies[menuIndex];
                                    bool isVisibleAlpha = (enemy.imageAlpha > 0.5f);
                                    ...
                                    if (content.growthFactor)
                                    {
                                        if (GetSliderValue(enemy, no, catalog, owner))
                                        {
                                            if (isHP.Value)
                                            {
                                                var guid = gs.maxHPStatusID;
                                                var hp = enemy.battleStatusData.consumptionStatusValue.GetStatus(guid);
                                                owner.ChangeDrawableParentContainer(isVisibleAlpha && hp > 0);
                                            }
                                            else
                                            {
                                                owner.EnableDrawable(isVisibleAlpha);
                                            }
                                        }
                                        else
                                        {
                                            if (isHP.Value) owner.ChangeDrawableParentContainer(false);
                                            else            owner.EnableDrawable(false);
                                        }
                                    }
                                    else
                                    {
                                        // MAXが0だった場合、ゲージが空っぽで表示できるようにする
                                        // If MAX is 0, the gauge will be displayed empty.
                                        var val = isHP.Value ? enemy.battleStatusData.HitPoint : enemy.battleStatusData.MagicPoint;
                                        var max = isHP.Value ? enemy.MaxHitPoint : enemy.MaxMagicPoint;
                                        ...
                                        if (isHP.Value) owner.ChangeDrawableParentContainer(isVisibleAlpha && val > 0);
                                        else            owner.EnableDrawable(isVisibleAlpha);
                                    }
                                }
                                else
                                {
