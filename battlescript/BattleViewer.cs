...
namespace Yukar.Battle
{
    ...
    public class BattleViewer
    {
        ...
        internal Dictionary<BattleCharacterBase, BattleCharacterBase> reflectionSourceList;
        ...
        public BattleViewer(GameMain owner)
        {
            ...
            reflectionSourceList = new Dictionary<BattleCharacterBase, BattleCharacterBase>();
        ...
        internal virtual void Draw(List<BattlePlayerData> playerData, List<BattleEnemyData> enemyMonsterData)
        {
            ...
            // Pokemon-style: Show ALL available targets when targeting (including reserves)
            if (displayWindow == WindowType.CommandTargetPlayerListWindow && commandTargetSelector != null)
            {
                var targetList = commandTargetSelector.GetTargetList(); // We'll need to expose this
                for (int i = 0; i < targetList.Count; i++)
                {
                    var targetPlayer = targetList[i] as BattlePlayerData;
                    if (targetPlayer != null && !targetPlayer.IsBattle) // Reserve members
                    {
                        Vector2 statusWindowSize = StatusWindowSize;
                        
                        // Ensure the reserve member has battle status data
                        if (targetPlayer.battleStatusData == null)
                        {
                            targetPlayer.battleStatusData = new BattleStatusWindowDrawer.StatusData();
                            targetPlayer.battleStatusData.Name = targetPlayer.Name;
                            targetPlayer.battleStatusData.HitPoint = targetPlayer.HitPoint;
                            targetPlayer.battleStatusData.MaxHitPoint = targetPlayer.MaxHitPoint;
                            targetPlayer.battleStatusData.MagicPoint = targetPlayer.MagicPoint;
                            targetPlayer.battleStatusData.MaxMagicPoint = targetPlayer.MaxMagicPoint;
                        }

                        Color drawColor = Color.White;
                        if (targetPlayer.IsSelect)
                        {
                            drawColor = blinker.getColor(); // Highlight selected
                        }
                        else if (commandTargetSelector.IsInTargetList(targetPlayer))
                        {
                            drawColor = new Color(200, 200, 255); // Light blue for available targets
                        }

                        battleStatusWindowDrawer.Draw(targetPlayer.battleStatusData, targetPlayer.statusWindowDrawPosition, statusWindowSize, drawColor);
                    }
                }
            }
        ...
        internal void AddReflectionSourceList(BattleCharacterBase target)
        {
