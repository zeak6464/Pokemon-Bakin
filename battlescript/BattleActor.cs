...
namespace Yukar.Battle
{
    ...
    public class BattleActor
    {
        ...
        private float actorOpacity = 1;
        private float conditionOpacity = 1;
        ...
        internal static BattleActor GenerateEnemy(Common.Catalog catalog, BattleEnemyData chr, int count, int max)
        {
            var result = new BattleActor();
            var mapChr = new MapCharacter(null);
            mapChr.guId = Guid.NewGuid();
            mapChr.ChangeGraphic(chr.image, null);
            //mapChr.setHeroSymbol(true);
            mapChr.useOverrideColor = true;
            mapChr.ChangeColor(255, 255, 255, 255);
            mapChr.setDisplayID(Common.Util.BATTLE3DDISPLAYID);
        ...
        internal void Update(MapData drawer, float yangle, bool isLockDirection)
        {
            ...
            {
                var stepCount = Math.Min(ESCAPE_MAX_COUNT, stateCount) / ESCAPE_MAX_COUNT;
                setActorOpacity(1.0f - stepCount);
                if (stepCount == 1.0)
                    mapChr.hide |= MapCharacter.HideCauses.BY_BATTLE;
            }
        ...
        {
            mapChr.setOpacityMultiplier(actorOpacity * conditionOpacity);
        }

        private void setActorOpacity(float v)
        {
            actorOpacity = v;
        ...
        internal void setConditionOpacity(float v)
        {
            conditionOpacity = v;
