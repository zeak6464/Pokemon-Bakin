...
namespace Yukar.Battle
{
    ...
    public class ResultViewer
    {
        ...
        internal void SetResultData(BattlePlayerData[] partyPlayer, int exp, int money, Rom.NItem[] items, Dictionary<Guid, int> itemDictonary)
        {
            ...
            // Pokemon-style: Debug log to verify all party members are included
            GameMain.PushLog(DebugDialog.LogEntry.LogType.BATTLE, "ResultViewer", 
                string.Format("Setting result data for {0} party members", partyPlayer.Length));
            for (int i = 0; i < partyPlayer.Length; i++)
            {
                GameMain.PushLog(DebugDialog.LogEntry.LogType.BATTLE, "ResultViewer", 
                    string.Format("Party Member {0}: {1} (HP: {2}/{3})", i, partyPlayer[i].Name, partyPlayer[i].HitPoint, partyPlayer[i].MaxHitPoint));
            }
            ...
            foreach (var player in partyPlayer)
            {
                ...
                data.castLevelUpData.preLevel = -1;
                ...
                else
                {
                    ...
                    data.jobLevelUpData.preLevel = -1;
                }
                ...
                else
                {
                    ...
                    data.sideJobLevelUpData.preLevel = -1;
                }
				...
				data.characterStatsCache = new CharacterStatsCache();
            ...
            // Pokemon-style: Arrange ALL party members in a grid layout on results screen
            for (int i = 0; i < partyPlayer.Length; i++)
            {
                var player = partyPlayer[i];
                
                // If player has a valid position from battle, use it; otherwise, create a new position
                Vector2 position;
                if (player.statusWindowDrawPosition != Vector2.Zero)
                {
                    // Active player - use their battle position
                    position = player.statusWindowDrawPosition;
                }
                else
                {
                    // Reserve player - create a position in a grid layout
                    // Arrange characters in rows of 2 across the screen
                    int row = i / 2;
                    int col = i % 2;
                    position = new Vector2(
                        300 + (col * 400), // Spread horizontally: 300, 700
                        200 + (row * 120)  // Spread vertically: 200, 320, 440, etc.
                    );
                }
                
                drawPosition.Add(position);
            ...
            switch (resultState)
            {
                ...
                case ResultState.AddExp:
                    ...
                    else
                    {
                        ...
                        for (int index = 0; index < characterLevelUpData.Length; index++)
                        {
                            ...
                            characterLevelUpData[index].setAddExpScaleFactor((float)countupExp / characterLevelUpData[index].castLevelUpData.addExp * characterLevelUpData[index].expRate / 100);
                ...
                case ResultState.CheckLevelUp:
                    foreach (var index in levelUpIndices.ToArray())
                    {
                        ...
                        characterLevelUpData[index].castLevelUpData.preLevel = characterLevelUpData[index].castLevelUpData.CurrentLevel;
                        characterLevelUpData[index].jobLevelUpData.preLevel = characterLevelUpData[index].jobLevelUpData.CurrentLevel;
                        characterLevelUpData[index].sideJobLevelUpData.preLevel = characterLevelUpData[index].sideJobLevelUpData.CurrentLevel;
        ...
        private void DrawFor3D()
        {
            ...
            // Pokemon-style: Debug log character rendering
            if (characterLevelUpData != null)
            {
                GameMain.PushLog(DebugDialog.LogEntry.LogType.BATTLE, "ResultViewer", 
                    string.Format("DrawFor3D: Rendering {0} characters", characterLevelUpData.Length));
            }

            for (int index = 0; index < (characterLevelUpData == null ? 0 : characterLevelUpData.Length); index++)
            {
                // Pokemon-style: Show ALL characters on results screen, not just those leveling up
                var characterData = characterLevelUpData[index];
                
                // Always show level up information if currently in effect
                if (characterData.onLevelUpEffect)
                {
                    // レベルアップウィンドウ
                    // Level up window
                    Vector2 levelupWindowOriginalSize = new Vector2(192, 32);
                    ...
                    var levelupWindowPosition = characterData.player.statusWindowDrawPosition - new Vector2(levelupWindowSize.X / 2, 0);

                    Vector2 skillWindowOffset = new Vector2(0, 40);

                    levelupWindowDrawer.Draw(levelupWindowPosition, levelupWindowSize);

                    if (characterData.onLevelUpText) textDrawer.DrawStringSoloColor(glossary.battle_levelup, levelupWindowPosition, (int)levelupWindowSize.X, TextDrawer.HorizontalAlignment.Center, Color.Red);
                    //if (characterData.onLevelUpText) Graphics.DrawImage(levelUpImageId, (int)levelupWindowPosition.X, (int)levelupWindowPosition.Y);
                    // スキルウィンドウ
                    // skill window
                    Vector2 skillWindowOriginalSize = new Vector2(192, 32);
                    Vector2 skillWindowPosition = levelupWindowPosition;

                    foreach (var skillName in characterData.learnSkillNames)
                    {
                        skillWindowPosition -= skillWindowOffset;
                    }
                }
                
                // Pokemon-style: Always draw character status for ALL party members
                var statusWindowColor = characterData.player.IsDeadCondition() ? Color.Red : Color.White;
                if (resultStatusWindowDrawer != null && index < resultStatusDrawPosition.Length && index < resultStatusWindowSize.Length)
                {
                    GameMain.PushLog(DebugDialog.LogEntry.LogType.BATTLE, "ResultViewer", 
                        string.Format("Drawing character {0}: {1} at position ({2}, {3})", 
                        index, characterData.player.Name, resultStatusDrawPosition[index].X, resultStatusDrawPosition[index].Y));
                    resultStatusWindowDrawer.Draw(characterData.resultStatus, resultStatusDrawPosition[index], resultStatusWindowSize[index], statusWindowColor);
                }
                else
                {
                    GameMain.PushLog(DebugDialog.LogEntry.LogType.BATTLE, "ResultViewer", 
                        string.Format("SKIP drawing character {0}: {1} - drawer={2}, posCount={3}, sizeCount={4}", 
                        index, characterData.player.Name, resultStatusWindowDrawer != null, 
                        resultStatusDrawPosition?.Length ?? 0, resultStatusWindowSize?.Length ?? 0));
                }
            }
